...
Socket::Socket(esl::object::ObjectContext& aEngineContext, const std::string& aId, esl::messaging::server::Interface::Socket& aSocket)
: engineContext(aEngineContext),
  socket(aSocket),
  id(aId)
{
	getSocket().addObjectFactory("", [this](const esl::messaging::server::RequestContext&) {
...
: engineContext(aEngineContext),
  socketPtr(new esl::messaging::server::Socket(aPort, esl::object::Properties(settings), aImplementation)),
  socket(*socketPtr),
  id(aId),
...
void Socket::addListener(Listener& aListener) {
	if(listener) {
		if(socketPtr) {
			throw esl::addStacktrace(std::runtime_error("There is already a listener connected to message-sever \"" + getId() + "\"."));
		}
		else {
			throw esl::addStacktrace(std::runtime_error("There is already a listener connected to message-broker \"" + getId() + "\"."));
		}
	}
	listener = &aListener;
}

esl::messaging::server::Interface::Socket& Socket::getSocket() const noexcept {
	return socket;
}
...
esl::io::Input Socket::createMessageHandler(esl::messaging::server::RequestContext& baseRequestContext) {
	/* Access log */
	...
	if(!socket) {
		logger.info << "ERROR: No message-server or message-broker available for request.\n";
		return esl::io::Input();
	}

	if(socket->socketPtr) {
		logger.debug << "Message-server found with id " << socket->getId() << "\"\n";
	}
	else {
		logger.debug << "Message-broker found with id " << socket->getId() << "\"\n";
	}
