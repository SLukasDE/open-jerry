...
public:
	Engine(std::function<void()> onProcessRegister, std::function<void()> onProcessUnregister);
	...
	void addBatchProcedure(const std::vector<std::pair<std::string, std::string>>& settings, const std::string& implementation);
	void setBatchProcedure(esl::processing::procedure::Interface::Procedure* batchProcedure);
	esl::processing::procedure::Interface::Procedure* getBatchProcedure() const noexcept;

	void addDaemonProcedure(const std::vector<std::pair<std::string, std::string>>& settings, const std::string& implementation);
...
private:
	std::function<void()> onProcessRegister;
	std::function<void()> onProcessUnregister;

	std::map<std::string, std::pair<std::vector<unsigned char>, std::vector<unsigned char>>> certsByHostname;

	std::unique_ptr<esl::processing::procedure::Interface::Procedure> batchProcedure;
	esl::processing::procedure::Interface::Procedure* batchProcedurePtr = nullptr;
