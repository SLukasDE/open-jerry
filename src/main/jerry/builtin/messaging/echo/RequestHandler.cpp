...
namespace {
Logger logger("jerry::builtin::messaging::echo::RequestHandler");
}
...
RequestHandler::RequestHandler(esl::messaging::server::RequestContext& aRequestContext, Settings& aSettings)
...
bool RequestHandler::consume(esl::io::Reader& reader) {
	...
	if(len == esl::io::Reader::npos) {
		...
		std::unique_ptr<esl::messaging::client::Interface::Connection> connection = settings.createConnection();
		if(connection) {
			std::unique_ptr<esl::io::Producer> producer(new esl::io::output::String(std::move(message)));

			logger.info << "Echo - send echo\n";
			connection->sendMessage(esl::io::Output(std::move(producer)));
		}
		else {
