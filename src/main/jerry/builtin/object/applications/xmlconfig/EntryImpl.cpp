...
: Entry(fileName, element)
{
	if(element.Name() == nullptr) {
		throw config::FilePosition::add(*this, "Element name is empty");
	}

	std::string elementName(element.Name());

	if(elementName == "object") {
		object = std::unique_ptr<config::Object>(new config::Object(getFileName(), element));
	}
	else if(elementName == "reference" || elementName == "alias") {
		reference = std::unique_ptr<config::Reference>(new config::Reference(getFileName(), element));
	}
	else if(elementName == "database") {
		database = std::unique_ptr<config::Database>(new config::Database(getFileName(), element));
	}
		...
		basicClient = std::unique_ptr<config::basic::Client>(new config::basic::Client(getFileName(), element));
	}
		...
		httpClient = std::unique_ptr<config::http::Client>(new config::http::Client(getFileName(), element));
	}
		...
		throw config::FilePosition::add(*this, "Unknown element name \"" + elementName + "\"");
	}
}
...
void EntryImpl::install(Application& engineApplicationContext) const {
	if(object) {
