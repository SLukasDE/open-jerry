...
void ProcedureContext::install(Application& engineApplicationContext) const {
	if(getRefId().empty()) {
		...
		if(getId().empty()) {
			engineApplicationContext.addProcedureContext(std::move(context));
		}
		else {
		...
		engineApplicationContext.addProcedureContext(getRefId());
	}
}
