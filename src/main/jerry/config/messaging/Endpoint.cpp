...
Endpoint::Endpoint(const tinyxml2::XMLElement& element) {
	...
	for(const tinyxml2::XMLAttribute* attribute = element.FirstAttribute(); attribute != nullptr; attribute = attribute->Next()) {
		if(std::string(attribute->Name()) == "queue") {
			hasQueue = true;
			queue = attribute->Value();
		}
		else {
	...
	if(hasQueue == false) {
		throw esl::addStacktrace(std::runtime_error(std::string("Missing attribute \"queue\" at line ") + std::to_string(element.GetLineNum())));
	}
	...
	for(const tinyxml2::XMLNode* node = element.FirstChild(); node != nullptr; node = node->NextSibling()) {
		...
		*/
		entries.push_back(Entry(*innerElement, true));
	}
}

void Endpoint::save(std::ostream& oStream, std::size_t spaces) const {
	oStream << makeSpaces(spaces) << "<endpoint queue=\"" << queue << "\">\n";
