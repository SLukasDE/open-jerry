...
MessageHandler::MessageHandler(const tinyxml2::XMLElement& element) {
...
void MessageHandler::save(std::ostream& oStream, std::size_t spaces) const {
	oStream << makeSpaces(spaces) << "<messagehandler implementation=\"" << implementation << "\"";
	...
	if(settings.empty()) {
		...
		oStream << makeSpaces(spaces) << "</messagehandler>\n";
	}
}
