...
#include <jerry/config/main/Applications.h>
...
namespace {
Logger logger("jerry::config::main::Applications");
} /* anonymous namespace */
...
: Config(fileName, element)
{
	...
	for( boost::filesystem::directory_iterator iter(path); iter != endIter; ++iter) {
		...
		applications.emplace_back(new application::Context(iter->path()));
	}
}
...
void Applications::install(engine::ObjectContext& engineObjectContext) const {
	std::unique_ptr<engine::main::Applications> engineApplications(new engine::main::Applications(engineObjectContext));
